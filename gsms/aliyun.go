...
type (
	aliyunSms struct {
		...
		TemplateParam    string `form:"TemplateParam" json:"TemplateParam"`       //短信模板中的变量；数字需要转换为字符串
...
func (s *aliyunSms) Send(mobiles string) (*SmsResult, error) {
	...
	if len(s.TemplateCode) == 0 || len(s.TemplateParam) == 0 || len(s.SignName) == 0 {
	...
	//获取参数字符串，然后附加签名字符串
	params := "Signature=" + s.Signature + "&" + s.GetParamString(false)

	geteway := "https://dysmsapi.aliyuncs.com"
	if len(s.Geteway) > 0 {
		geteway = s.Geteway
	}

	fmt.Println("post : %s", params)
...
func (s *aliyunSms) toDict() map[string]string {
	...
	params["TemplateParam"] = s.TemplateParam
